...
class Admin_model extends CI_model
{
   ...
   public function get_vendor_store($aid)
   {
      $this->db->select('restaurants.*');
      $this->db->from('restaurants');
   ...
   public function get_new_booking()
   {
   ...
   public function get_new_service_review()
   {
      $this->db->select('services_reviews.*');
      $this->db->from('services_reviews');
      $this->db->order_by("services_reviews.rev_id", "desc");
      $this->db->limit(5);
      $query = $this->db->get();
      return $query->result();
   }
   ...
   public function get_vendor_booking($aid)
   {
      $this->db->select('booking.*');
      $this->db->from('booking');
      $this->db->where("vid", $aid);
   ...
   public function get_all_vendor_product($aid)
   {
      $this->db->select('products.*');
      $this->db->from('products');
      $this->db->where("vid", $aid);
   ...
   public function get_all_vendor_orders($aid)
   {
   ...
   {
      return $this->db->get_where('orders', array('id' => $id), 1)->row();
   }

   public function get_order_items_by_id($id)
   {
      return $this->db->order_by('id', 'DESC')->get_where('order_items', array('order_id' => $id))->result();
   }
   ...
   public function get_all_vendor_service($aid)
   {
      $this->db->select('services.*');
      $this->db->from('services');
      $this->db->where("v_id", $aid);
      $this->db->order_by("id", "desc");
      $query = $this->db->get();
      return $query->result();
   }

   public function get_all_service($res_id)
   {
      $this->db->select('services.*');
      $this->db->from('services');
   ...
   {
      $res = $this->db->update('services', $data, ['id' => $id]);
      if ($res == 1)
         return true;
      else
         return false;
